...
const check_emoji = (j) => {
  const containsEmoji = (text) => {
    ...
    let res = [...text.toString()].filter(e => {
      let t = !([...'0123456789#*©'].includes(e)) && emojiRegex.includes(e.codePointAt())
      return t;
    });
    return res.length > 0;
  }
  try {
    __emoji_text = containsEmoji(j)
    if (__emoji_text) _tmp_class.value += " emoji_text";
  } catch { }
  // // ios 兼容
  // try {
